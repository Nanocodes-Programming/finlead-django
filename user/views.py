...
# from .forms import FaceLoginForm
...
#     #     # user_db = User.objects.get(user=username)
#     #     profile = get_object_or_404(Profile, user=user_db)
#     #     #profile = Profile.objects.get(user=user_db)

#     #     # Get the image associated with the user
#     #     uploaded_image = profile.face_image
#         form = FaceLoginForm(request.POST)
#         if form.is_valid():
...
#         face_cascade = cv2.CascadeClassifier(r'user/haarcascade.xml')
...
#             file_path = os.path.join(settings.MEDIA_ROOT, 'face_reg_images/capture_to_verify.jpg')
#             # If a face is detected, save the frame to a file
#             if len(faces) > 0:
...
#         image2 = os.path.join("media/face_reg_images/capture_to_verify.jpg")
#         from . import face_recog
#         result = face_recog.compare_old_and_new_face(uploaded_image, image2)
#         if result == [True]:
...
#     else:
#         form = FaceLoginForm()
#     return render(request, 'account/face_login.html', {'form' : form})
